...
- **`/read-design`** — list or read Design Systems through `afk-research design list/read`, without prompt-side selection menus.
- **`/delete-design`** — delete a selected Design System or all Design Systems through `afk-research design delete`, preserving the Design Workspace container.
...
- **`/save`** — stage the current repository changes, commit them, and push the commit to the configured remote. Reach for it when the user wants the latest work preserved in the remote repository.
- **`/update-setup`** — guide a dry-run-confirmed AFK Research Setup Update through the CLI. Reach for it when the user wants to refresh AFK-managed setup files or install the latest Bundled Project Skills from the Update Source.
