...
8. **`scatter_vandermeijde.png`** — Depth–depth comparison (van der Meijde 2013 style): open circles vs the 1:1 line with ±6 km (black) and ±12 km (red) deviation bands. (a) This study, (b) CRUST1.0, (c) GEMMA vs the 105 seismic depths; (d) GEMMA vs CRUST1.0 over the model grid (N=817). Insets give bias, RMS, % within ±6/±12 km and slope. This study (slope 0.20) and CRUST1.0 (slope 0.23) show the same amplitude compression — evidence it stems from the reference data/scale mismatch, not our method — while GEMMA (slope 0.08) is systematically shallow. [Also available: `scatter_vs_seismic.png`, the single-panel overlay with regression line and binned means.]
