...
$\pm10$~km, respectively (no points were excluded as outliers). The correlation
against the seismic depths is modest ($r=0.28$) because the RF depths span only a
narrow $20$--$40$~km range and are spatially clustered, which compresses the
dynamic range available to the correlation coefficient.
...
stations for this study, {CRUST1.0} and {GEMMA} (dashed $1{:}1$ line). This study
and {CRUST1.0} scatter about the line; {GEMMA} lies systematically below it (too
shallow). Correlations are given in the legend and Table~\ref{tab:compare}.}
\label{fig:scatter}
\end{figure}
