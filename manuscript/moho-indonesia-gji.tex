...
{GEMMA} almost everywhere. This benchmark substantiates the claim that existing
...
deviation bands. (a) This study, (b) {CRUST1.0} and (c) {GEMMA} versus the 105
seismic Moho depths; (d) {GEMMA} versus {CRUST1.0} over the model grid ($N=817$).
Panel statistics (bias, RMS, per cent within $\pm6$/$\pm12$~km, and the regression
slope) are inset. This study (slope $0.20$) and {CRUST1.0} (slope $0.23$) show the
\emph{same} amplitude compression --- evidence it stems from the point-vs-footprint
scale mismatch and the narrow RF depth range, not from our method --- while
{GEMMA} (slope $0.08$) is systematically shallow.}
\label{fig:scatter}
\end{figure*}
