...
## [2026-07-30] maintenance | Δρ runaway finding + adopted-point diagnostic figure

- **Trigger:** user asked to extend the Δρ axis so the triangle sits inside the frame (like U&B Fig.10), and whether Δρ=500 (> U&B's ~400) makes sense.
- **Finding (important):** extending DRHO_VALUES to 600 shows the seismic misfit **decreases monotonically with Δρ** (500→5.83, 550→5.64, 600→5.51 km) — the calibration argmin RUNS AWAY to unphysical Δρ. Classic gravity non-uniqueness: Δρ trades off with z_ref/Moho amplitude and absorbs unmodelled effects (sediments, slab, constant height, μ≈0). So Δρ=500 is an *effective* contrast, not tightly determined; we adopt a physical Δρ=500 (cf. U&B 400) + z_ref=35, confirmed by AusMoho (bias -0.1 km; runaway solutions degrade the independent agreement).
- **Figure:** `moho_indonesia/plot_hyperparam_diag.py` re-plots the U&B-style diagnostic marking the ADOPTED (35, 500) point (now inside the extended Δρ≤600 frame); reads `hyperparameters_sweep.npz` (saved by calibrate --fine) or reconstructs from a calibrate log. calibrate.py now saves the sweep npz and calls this plotter.
- **Featured model restored** to z_ref=35, Δρ=500 via `calibrate --only-final` (the --fine sweep had set the runaway 32.5/600). Manuscript §4 updated with the runaway/non-uniqueness discussion.

## [2026-07-30] query | Tested RF (Bahri) sediment correction — over-corrects, does not help

- **Trigger:** user provided RF sediment thickness + Moho from Syaiful Bahri's MSc thesis (Appendix A, 91 western-Indonesia stations) and asked to test the sediment correction.
- **Data:** extracted to `data/external/rf_sediment_moho_bahri.txt` (station lon lat sed_km moho_km). Sediment thickness is coarsely QUANTIZED (79/91 = 2 km; a few 6-10 km); RF Moho also quantized and NOISY (r=0.04 vs our model — worse than Depth_Moho.txt, so unusable as validation).
- **Test (`moho_indonesia/test_rf_sediment.py`):** interpolated sediment to grid (linear-in-hull, 0 outside), fixed density 2320 kg/m^3, tesseroid effect (-116..0 mGal), re-inverted vs baseline (same hyperparameters).
- **Result:** the correction changes the Moho (west mean -0.78 km, local max 13.6 km = interpolation artefact) but WORSENS the seismic fit: overall std 5.75 -> 6.10; the WEST subset was already near-perfect without sediment (mean +0.03, std 5.49) and the correction pushed it to mean -1.91 -> OVER-CORRECTION.
- **Conclusion:** RF station-point sediment (thin, quantized, sparse) is not useful to improve the gravity Moho; the baseline already matches the western seismic Moho. Confirms sediments are not the limiting factor (cf. CRUST1.0 null result + Δρ runaway). Good negative result to cite; real help would need basin-depocenter sediment maps (Hardy 1997 / ESDM 2022), not station RF.
